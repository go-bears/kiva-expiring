...
def preprocess_json(json_response_list):
    ...
    for loan in json_response_list:
        ...
        loan['loanFundraisingInfo']['fundedAmount'] = float(loan['loanFundraisingInfo']['fundedAmount'])
        loan['link'] = LEND_TAB_URL_BASE + str(loan['id'])
        loan['amtLeftToFundraise']  = loan['loanAmount'] - loan['loanFundraisingInfo']['fundedAmount']
        loan['plannedExpirationDate'] = parser.parse(loan['plannedExpirationDate'])
...
def show_total_fundraising_needed(subset_24_hrs_list):
    ...
    if subset_24_hrs_list:
        ...
        print("Total amount needed to fund loans expiring within 24 hours: {0:.2f} \n".format(total))
...
def show_expiring_loans(subset_24_hrs_list):
    ...
    for loan in subset_24_hrs_list:
        if loan['amtLeftToFundraise'] > 0:
            print (loan['link'], "amount left to raise: {0:.2f}".format(loan['amtLeftToFundraise'] ))
...
def main():
	...
	# print(len(test2))
	show_total_fundraising_needed(test2)
